...
segments <- segments %>%
  ...
  for (segment_idx in 1:n_segments) {
    ...
    if (k %in% possible_k) {
      ...
      if (nrow(accepted_mutations) >= min_mutations_number) {
        ...
        comp_binomial <- fit$draws("comp_binomial", format = "matrix") %>% as.numeric()   
        yrep <- fit$draws("y_rep", format = "matrix") %>% as.numeric()   
        ...
        summarized_results <- dplyr::bind_rows(summarized_results, dplyr::tibble(omega_1 = mean(omega1), omega_2 = mean(omega2), tau_low = tau_low, tau_mean = tau_mean, tau_high = tau_high, segment = segment_name, karyotype = k, chr = chr, segment_id = segment_id))
        component_binomial <- dplyr::bind_rows(component_binomial, dplyr::tibble(comp_binomial = comp_binomial[1:nrow(accepted_mutations)],  segment = segment_name, karyotype = k, chr = chr, segment_id = segment_id))
        y_rep <- dplyr::bind_rows(y_rep, dplyr::tibble(yrep = yrep[1:nrow(accepted_mutations)],  segment = segment_name, karyotype = k, chr = chr, segment_id = segment_id))
        omega_1 <- dplyr::bind_rows(omega_1, dplyr::tibble(omega1 = omega1,  segment = segment_name, karyotype = k, chr = chr, segment_id = segment_id))
        omega_2 <- dplyr::bind_rows(omega_2, dplyr::tibble(omega2 = omega2,  segment = segment_name, karyotype = k, chr = chr, segment_id = segment_id))
  ...
  return(list(omega_1 = omega_1, omega_2 = omega_2 , y_rep = y_rep, component_binomial = component_binomial, inference_results = inference_results, summarized_results = summarized_results, accepted_mutations=accepted_mutations_all ))
}
